...
- `/specops` — main spec-driven development workflow (from `skills/specops/SKILL.md`)
- `/specops:init` — interactive `.specops.json` configuration setup (from `skills/init/SKILL.md`)
...
`skills/specops/` and `skills/init/` are generated by the build system and used for plugin discovery. The canonical sources are in `platforms/claude/`. Both locations are kept in sync by the generator.
...
- **Init skill** — `skills/init/SKILL.md` exists, has frontmatter, contains all 5 config templates, matches `platforms/claude/init/SKILL.md`
