...
3. **Collect approved dependencies**: READ_FILE(`<specsDir>/index.json`) to enumerate all specs. For each spec with `status == "completed"`, READ_FILE(`<specsDir>/<spec-name>/design.md`) and extract packages from the `### Dependency Decisions` table where `Decision` is `Approved`. Build a union set of all approved packages across all completed specs.
