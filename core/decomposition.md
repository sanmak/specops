...
1. If `scope_cut`: Update requirements.md and tasks.md to remove the blocked functionality. READ_FILE spec.json, remove the dependency entry from `specDependencies` (or set `required: false`), WRITE_FILE spec.json. Proceed to Phase 3 with reduced scope.
2. If `interface_defined`: WRITE_FILE the interface contract. READ_FILE spec.json, update the specDependency entry's `contractRef` field with the contract path, WRITE_FILE spec.json. Proceed to Phase 3 with stub implementation.
