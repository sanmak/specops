...
1. READ_FILE(`.specops.json`) at the project root.
...
3. **tasks.md exists**: FILE_EXISTS(`<specsDir>/<spec-name>/tasks.md`). If false, NOTIFY_USER("tasks.md not found for spec '<spec-name>'. The spec may be incomplete — run the spec workflow to generate it.") and STOP.
...
8. **Spec dependency gate**: READ_FILE(`<specsDir>/<spec-name>/spec.json`) and check the `specDependencies` array. For each entry with `required: true`, READ_FILE(`<specsDir>/<entry.specId>/spec.json`) and verify `status == "completed"`. If any required dependency is not completed, NOTIFY_USER("Spec '<spec-name>' has unmet required dependency: '<entry.specId>' (status: <status>). Complete the dependency spec first, or use scope hammering to resolve the blocker.") and STOP. Additionally, run cycle detection (DFS with white/gray/black coloring) across all specs' `specDependencies` — if a cycle is detected, NOTIFY_USER("Circular dependency detected: {cycle chain}. Resolve before proceeding.") and STOP. If `specDependencies` is absent or empty, this check passes trivially.
...
- READ_FILE on user-supplied paths: validate relative path, containment under repo root, and absence of ../ traversal sequences.
...
When READ_FILE is called on a user-supplied path (e.g., plan file paths in from-plan mode), validate:
...
If any check fails, reject the path and NOTIFY_USER with the specific violation.
