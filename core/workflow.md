...
5.8. **Dependency introduction gate**: **Depth calibration**: If the depth flag is `lightweight` and no new dependencies are detected in design.md (no install commands or new package references found during scanning), skip this gate — it passes trivially. For `standard` and `deep` depth, execute normally. For `deep` depth, extend the evaluation with additional analysis: check all transitive dependencies of net-new packages for known issues. Execute the Phase 2 Gate Procedure from the Dependency Introduction Gate module (`core/dependency-introduction.md`). Scan design.md for install commands and new package references, compare against the Detected Dependencies in `dependencies.md`, evaluate net-new dependencies using the Build-vs-Install framework and Maintenance Profile Intelligence, surface each to the user via ASK_USER, and record all decisions in design.md under `### Dependency Decisions`. Update the Dependency Introduction Policy in `dependencies.md`. If no new dependencies are found, the gate passes trivially. This gate is always active -- there is no config switch to disable it.
...
1. **Implementation gates** — run these checks before any implementation begins:
   ...
   1.5. **Environment pre-flight**: Before updating status, run lightweight environment checks to catch problems early. These checks are unconditional (no config switch to disable):
      - **Test command detection**: Detect the project's test command by checking project config files in priority order:
        1. If FILE_EXISTS(`package.json`), READ_FILE it. If it contains a `"test"` script under `"scripts"`, use that command.
        2. Else if FILE_EXISTS(`pyproject.toml`), READ_FILE it. If it contains `[tool.pytest]`, use `pytest`. If it contains `[tool.unittest]`, use `python -m unittest`.
        3. Else if FILE_EXISTS(`Makefile`), READ_FILE it. If it contains a `test:` target, use `make test`.
        4. Else if FILE_EXISTS(`Cargo.toml`), use `cargo test`.
        5. Else if FILE_EXISTS(`go.mod`), use `go test ./...`.
        6. Else: NOTIFY_USER("No test command detected -- manual test execution required during Phase 3.") and set test_cmd to null.
      ...
      - **Dependency installation check**: Based on the project type detected above, check for the expected dependency directory:
        - npm/yarn: FILE_EXISTS(`node_modules/`). If missing, NOTIFY_USER("Dependencies may not be installed. Run `npm install` (or `yarn install` if FILE_EXISTS(`yarn.lock`)) before proceeding.").
        - Python: FILE_EXISTS(`.venv/`) or FILE_EXISTS(`__pycache__/`). If both missing, NOTIFY_USER("Python dependencies may not be installed. Run `pip install -r requirements.txt` or `pip install -e .` before proceeding.").
        - Rust: FILE_EXISTS(`target/`). If missing, NOTIFY_USER("Rust dependencies may not be built. Run `cargo build` before proceeding.").
        - Go (with vendor): FILE_EXISTS(`vendor/`). If missing, NOTIFY_USER("Go vendor directory missing. Run `go mod download` before proceeding.").
      ...
      - **Pre-flight summary**: NOTIFY_USER with a concise summary: "Environment pre-flight: Test command: [detected or none] | Dependencies: [OK or warning] | Git state: [clean / dirty / BLOCKED: conflicts]".
