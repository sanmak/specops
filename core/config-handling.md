...
1. RUN_COMMAND(`gh issue create --title "<taskPrefix><TaskTitle>" --body "<TaskDescription>"`)
2. Parse the issue URL/number from stdout
3. EDIT_FILE `tasks.md` — set the task's `**IssueID:**` to the returned issue identifier (e.g., `#42`)

**Jira** (`taskTracking: "jira"`):
1. RUN_COMMAND(`jira issue create --type=Task --summary="<taskPrefix><TaskTitle>" --description="<TaskDescription>"`)
2. Parse the issue key from stdout (e.g., `PROJ-123`)
3. EDIT_FILE `tasks.md` — set the task's `**IssueID:**` to the returned key

**Linear** (`taskTracking: "linear"`):
1. RUN_COMMAND(`linear issue create --title "<taskPrefix><TaskTitle>" --description "<TaskDescription>"`)
2. Parse the issue identifier from stdout
3. EDIT_FILE `tasks.md` — set the task's `**IssueID:**` to the returned identifier
