...
7. **Auto-fix** — if `config.dependencySafety.autoFix` is `true`:
   ...
   - Rust: Execute the command(`cargo update`)
...
- Execute the command(`curl -s --max-time 10 -X POST "https://api.osv.dev/v1/query" -H "Content-Type: application/json" -d '{"package":{"name":"<pkg>","ecosystem":"<ecosystem>"}}'`)
