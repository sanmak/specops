...
2. Write the file at `{tmpfile}` with the composed issue body.
3. Set the sanitized title in an environment variable: `SPECOPS_TITLE="[{category}] {sanitized_title}"`
4. Execute the command(`SPECOPS_TITLE="[{category}] {sanitized_title}" gh issue create --repo sanmak/specops --title "$SPECOPS_TITLE" --label "{label}" --body-file {tmpfile}`)
5. Execute the command(`rm -f {tmpfile}`) to clean up — always run this regardless of whether step 4 succeeded or failed.
...
8. Print to stdout("Feedback submitted: {issue URL}\n\nThank you for helping improve SpecOps!")
9. Stop.
