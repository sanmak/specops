...
5.8. **Dependency introduction gate**: **Depth calibration**: If the depth flag is `lightweight` and no new dependencies are detected in design.md (no install commands or new package references found during scanning), skip this gate — it passes trivially. For `standard` and `deep` depth, execute normally. For `deep` depth, extend the evaluation with additional analysis: check all transitive dependencies of net-new packages for known issues. Execute the Phase 2 Gate Procedure from the Dependency Introduction Gate module (`core/dependency-introduction.md`). Scan design.md for install commands and new package references, compare against the Detected Dependencies in `dependencies.md`, evaluate net-new dependencies using the Build-vs-Install framework and Maintenance Profile Intelligence, surface each to the user via If uncertain, note assumptions in the spec and proceed. List any ambiguities for the user to review, and record all decisions in design.md under `### Dependency Decisions`. Update the Dependency Introduction Policy in `dependencies.md`. If no new dependencies are found, the gate passes trivially. This gate is always active -- there is no config switch to disable it.
...
1. **Implementation gates** — run these checks before any implementation begins:
   ...
   1.5. **Environment pre-flight**: Before updating status, run lightweight environment checks to catch problems early. These checks are unconditional (no config switch to disable):
      - **Test command detection**: Detect the project's test command by checking project config files in priority order:
        ...
        6. Else: Print to stdout("No test command detected -- manual test execution required during Phase 3.") and set test_cmd to null.
      ...
      - **Dependency installation check**: Based on the project type detected above, check for the expected dependency directory:
        - npm/yarn: Check if the file exists at(`node_modules/`). If missing, Print to stdout("Dependencies may not be installed. Run `npm install` (or `yarn install` if Check if the file exists at(`yarn.lock`)) before proceeding.").
        - Python: Check if the file exists at(`.venv/`) or Check if the file exists at(`__pycache__/`). If both missing, Print to stdout("Python dependencies may not be installed. Run `pip install -r requirements.txt` or `pip install -e .` before proceeding.").
        - Rust: Check if the file exists at(`target/`). If missing, Print to stdout("Rust dependencies may not be built. Run `cargo build` before proceeding.").
        - Go (with vendor): Check if the file exists at(`vendor/`). If missing, Print to stdout("Go vendor directory missing. Run `go mod download` before proceeding.").
        This check is non-blocking -- warn and continue.
      - **Git branch state check**: Execute the command(`git status --porcelain`). Parse the output:
        - If the command fails (not a git repo), skip this check silently.
        - If any output lines start with `UU`, `AA`, `DD`, `AU`, or `UA`: merge conflicts detected. Print to stdout("Merge conflicts detected in: [file list]. Resolve conflicts before implementation.") and STOP -- do not proceed to implementation.
        - If output is non-empty but no conflict markers: Print to stdout("Working tree has uncommitted changes.") -- non-blocking warning, continue.
        - If output is empty: clean state.
      - **Pre-flight summary**: Print to stdout with a concise summary: "Environment pre-flight: Test command: [detected or none] | Dependencies: [OK or warning] | Git state: [clean / dirty / BLOCKED: conflicts]".
...
- Bare "explore" with no additional context routes to explore mode, which will If uncertain, note assumptions in the spec and proceed. List any ambiguities for the user to review for the problem statement.
...
2. If Check if the file exists at(`<specsDir>/steering/`), List the directory at(`<specsDir>/steering/`) and Read the file at each `.md` file with `inclusion: always` in its frontmatter. Store as project context.
...
1. **Codebase grounding requirement**: Each approach MUST reference at least 2 real files or directories from the repo map. If no repo map is available, Print to stdout("No repo map found. Generating approaches based on available project context. Run `/specops map` first for better-grounded suggestions.") and proceed with best-effort generation using any steering files and memory context available.
...
1. If `canAskInteractive` is true:
   If uncertain, note assumptions in the spec and proceed. List any ambiguities for the user to review("Which approach would you like to proceed with? Enter a number (1-N), 'more' for additional approaches, or 'refine' to adjust the problem statement.")
   ...
   - If user says "refine": If uncertain, note assumptions in the spec and proceed. List any ambiguities for the user to review("What would you like to adjust about the problem statement?"). Update the problem statement and return to `generating` phase.
...
2. If `canAskInteractive` is false:
   Print to stdout("Explore mode generated N approaches. Review the approaches above and re-invoke with your selected approach number to proceed to spec creation.")
...
2. Print to stdout("Proceeding to spec creation with the selected approach as design guidance...")
...
- If the repo map is stale (check `generated` timestamp if available), Print to stdout("Repo map may be outdated. Consider running `/specops map` for the most accurate file references.")
...
8. **Complete**: Proceed to Phase 2 spec review gate (if `config.team.specReview.enabled` or `config.team.reviewRequired`) or Print to stdout that the spec is ready and they can begin implementation.
...
- Node.js: Execute the command(`curl -s --max-time 10 "https://registry.npmjs.org/<package>"`) -- extract `time.modified`, version count, latest version date
- Python: Execute the command(`curl -s --max-time 10 "https://pypi.org/pypi/<package>/json"`) -- extract `info.version`, `releases` dates
- Other ecosystems: skip Layer 1, fall through to Layer 2

If the request times out or returns an error, Print to stdout("Registry query failed for <package> -- falling through to source repo check.") and proceed to Layer 2.

**Layer 2 -- Source Repository APIs:**

Query the source repository for activity metrics:

- If the package metadata includes a repository URL pointing to GitHub: Execute the command(`curl -s --max-time 10 "https://api.github.com/repos/<owner>/<repo>"`) -- extract `stargazers_count`, `pushed_at`, `open_issues_count`
- If no repository URL or not on GitHub: skip to Layer 3

If the request times out or returns an error, Print to stdout("Source repo query failed for <package> -- falling through to LLM assessment.") and proceed to Layer 3.
...
4. For each net-new dependency (maximum 10):
   ...
   c. If uncertain, note assumptions in the spec and proceed. List any ambiguities for the user to review with the evaluation output, asking for approval or rejection:
...
3. If an approved package was NOT installed: Print to stdout("Warning: dependency '<package>' was approved in design.md but was not installed during implementation. This may indicate a phantom approval or a change in approach. Please confirm this is intentional.") -- this is a warning, not a blocking error
...
- **`canAskInteractive = false`**: Present the evaluation and default to the recommendation. Record the recommendation as the decision. Print to stdout with the full evaluation output so the user can review.
...
4. Write the file at `<specsDir>/<spec-name>/evaluation.md` using the Evaluation Report Template. If the file already exists, append the new iteration (do not overwrite prior iterations).
...
8. If ANY dimension scores below `minScore` AND current iteration >= `maxIterations`: Print to stdout("Spec evaluation did not pass after {iterations} iterations. Proceeding to implementation with known spec gaps: {list of failing dimensions}.") and signal for Phase 3 dispatch with an incomplete evaluation flag.
...
3. If `canExecuteCode` is true AND `config.implementation.evaluation.exerciseTests` is true: Execute the command to execute the project's test suite. Record test output (pass count, fail count, specific failures).
...
6. **Score variance check**: After all dimensions are scored, check whether all dimension scores are identical.
   ...
   - If scores are still identical after one re-run, treat the evaluation as failed for this iteration and continue with normal iteration accounting (`maxIterations` applies).
7. Write the file at (or append to) `<specsDir>/<spec-name>/evaluation.md` with the implementation evaluation iteration. Append under the `## Implementation Evaluation` section.
...
11. If ANY dimension scores below `minScore` AND current iteration >= `maxIterations`: Print to stdout("Implementation evaluation did not pass after {iterations} iterations. Proceeding to completion with known quality gaps: {list of failing dimensions and scores}.") and proceed to Phase 4C.
...
2. For each auto_fix finding, in order:
   ...
   b. If `canExecuteCode` is true, Execute the command the project's test suite. If any test that was previously passing now fails, the auto-fix introduced a regression.
...
2. If `canAskInteractive` is true: present all gated fixes as a single batch to the developer via If uncertain, note assumptions in the spec and proceed. List any ambiguities for the user to review. For each finding, show: finding ID, description, proposed change (file path + change summary), and risk assessment. The developer can approve all, reject all, or selectively approve individual fixes.
...
1. `manual` findings: Print to stdout with each manual finding's ID, severity, confidence, evidence, and description. Note that no agent fix is attempted.
...
- Print to stdout("Evaluation feedback loop made no progress — scores unchanged after iteration {N}. Stopping to avoid infinite loop.")
...
2. Apply the adversarial spec evaluator against the collected artifacts and Write the file at the results to `<specsDir>/<spec-name>/evaluation.md`.
3. Read the file at(`<specsDir>/<spec-name>/evaluation.md`) and check the overall verdict. If the verdict is `fail`, Print to stdout("Spec evaluation failed before pipeline start. Review evaluation.md for findings.") and STOP — do not enter the cycle loop. If the verdict is `pass`, proceed to the cycle loop.
...
while cycle < maxCycles:
    ...
    if evaluationEnabled:
        ...
        if evaluationResponse is valid JSON with status, scores, verdict, findings:
            ...
            // Write the file at <specsDir>/<spec-name>/evaluation.md with formatted results
        else:
        ...
        if canAskInteractive:
            // Batch all gated_fix items and If uncertain, note assumptions in the spec and proceed. List any ambiguities for the user to review for approval
            // Apply approved fixes, reclassify rejected as manual
        else:
            // Non-interactive: treat gated_fix as auto_fix (apply without asking)

        // Report manual findings to developer via Print to stdout (no fix attempted)
