...
3. If FILE_EXISTS(`.specops.json`), Read the file at it and check for `_installedVersion` and `_installedAt` fields. If present, display:
...
3. Execute the command(`git config user.email`) to get the current user's email
4. Determine mode:
...
2. If FILE_EXISTS(`.specops.json`), Read the file at it and check for `_installedVersion` and `_installedAt` fields.
3. Display:
