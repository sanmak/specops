...
2. Create four core files:
   ...
   **Critical or High severity:**
   1. **Blast Radius Survey** — List the directory at and Read the file at the affected component's directory. Identify every module, function, or API that imports or calls the affected code. If Execute the command is available, search for usages across the codebase. Record each entry point in the Blast Radius subsection.
...
3. Write the file at(`<specsDir>/<spec-name>/spec.json`) with:
...
2. If spec review is enabled (`config.team.specReview.enabled` or `config.team.reviewRequired`):
   - If `status` is `"approved"` or `"self-approved"`: proceed with implementation. If `status` is `"self-approved"`, Print to stdout: "Note: This spec was self-approved without peer review." Set `status` to `"implementing"`, update `specopsUpdatedWith` to the current SpecOps version, update `updated` timestamp, regenerate `index.json`.
...
<!-- Survey what code paths are touched by the affected component(s).
     Use List the directory at and Read the file at to find callers, importers, and dependents.
     Use Execute the command to search for usages if the platform supports code execution.
