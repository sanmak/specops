...
- On interactive platforms (`canAskInteractive = true`), If uncertain, note assumptions in the spec and proceed. List any ambiguities for the user to review: "No steering files found. Would you like to create foundation steering files (product.md, tech.md, structure.md) for persistent project context?"
  - If yes: create the directory and 3 foundation templates using:
    ...
    (see Foundation File Templates above for `<...Template>` contents), then Print to stdout: "Created 3 steering files in `<specsDir>/steering/`. Edit them to describe your project — the agent will load them automatically before every spec."
...
2. **Discover project files**:
   - If `canAccessGit` is true: Execute the command(`git ls-files --cached --others --exclude-standard | head -200`) to get tracked and untracked-but-not-ignored files. This respects `.gitignore` natively.
...
3. **Apply scope limits**: Sort files alphabetically by path. If file count exceeds 100, keep the first 100 files and Print to stdout("Repo map scope limit: showing 100 of {total} files."). Exclude files deeper than 3 directory levels from the project root.
...
7. **Compute source hash**: If `canAccessGit` is true: Execute the command(`git ls-files --cached --others --exclude-standard | sort | sha256sum | cut -d' ' -f1`). If `canAccessGit` is false or the command fails, compute a simple hash by concatenating all sorted file paths and noting the count as the hash: `"manual-{file_count}-{first_path_hash}"`.

8. **Get timestamp**: Execute the command(`date -u +"%Y-%m-%dT%H:%M:%SZ"`) for the `_generatedAt` field.

9. **Write the repo map**: Write the file at(`<specsDir>/steering/repo-map.md`) with the frontmatter and body content assembled in the steps above.
...
|------|-----------|-----------|-------------------|-------------------|
| 1 | Python | `*.py` | Execute the command(`python3 -c "..."`) using `ast.parse()` | Top-level function signatures (`def name(args)`), class names (`class Name`) |
| 2 | TypeScript/JavaScript | `*.ts`, `*.tsx`, `*.js`, `*.jsx` | Execute the command(`grep -n "^export " <path> \| head -10`) | Export declarations (functions, classes, constants, types) |
| 3 | Go, Rust, Java | `*.go`, `*.rs`, `*.java` | Execute the command(`grep -n "^func \|^pub fn \|^fn \|^public class \|^public interface " <path> \| head -10`) | Top-level function/method/class declarations |
...
1. If FILE_EXISTS(`<specsDir>/steering/repo-map.md`):
   ...
   - If stale: Print to stdout("Repo map is stale ({reason}). Refreshing...") and run the Generation algorithm.
   - If fresh: the repo map was already loaded in step 3 as an `inclusion: always` steering file. Continue.

2. If the file does not exist:
   - On interactive platforms (`canAskInteractive = true`): If uncertain, note assumptions in the spec and proceed. List any ambiguities for the user to review("No repo map found. Would you like to generate one? This maps your project structure for better spec generation.")
     - If yes: run the Generation algorithm.
     - If no: continue without repo map.
   - On non-interactive platforms (`canAskInteractive = false`): Print to stdout("Tip: Run `/specops map` to generate a structural repo map for better spec context.") and continue without it.
...
2. If FILE_EXISTS(`<specsDir>/steering/repo-map.md`):
   ...
   - Display current map metadata:
     ...
     ```
   - On interactive platforms: If uncertain, note assumptions in the spec and proceed. List any ambiguities for the user to review("Refresh the repo map?")
...
| `canAccessGit: false` | Fall back to List the directory at for file discovery. Hash computed from file count + path list. |
...
| `canExecuteCode: true` (all platforms) | Execute the command available for `git ls-files`, `grep`, `python3`, `date`, `sha256sum`. |
...
On non-interactive platforms (`canAskInteractive = false`), the plan content must be provided inline. If not provided, Print to stdout: "From Plan mode requires the plan to be pasted inline. Re-invoke with your plan content included in the request." and stop.

## Workflow

1. **Receive plan content**: If plan content was provided inline with the invocation, use it directly. Otherwise, if `canAskInteractive`, If uncertain, note assumptions in the spec and proceed. List any ambiguities for the user to review: "Please paste your plan below." If `canAskInteractive` is false and no inline content was provided, Print to stdout: "From Plan mode requires the plan to be pasted inline. Re-invoke with your plan content included in the request." and stop.
