...
2. If FILE_EXISTS(`<specsDir>/steering/`):
   ...
   - If the number of files exceeds `config.steering.maxFiles` (default 20), Print to stdout("Steering file limit reached: loading first {maxFiles} of {total} files. Increase `steering.maxFiles` in .specops.json to load more.") and process only the first `maxFiles` files (sorted alphabetically by filename).
   - For each `.md` file:
...
3. After loading `always` files, Print to stdout with a brief summary: "Loaded {N} steering file(s): {names}"
