...
2. If `interface_defined`: Write the file at the interface contract, update `contractRef` in the specDependency entry, proceed to Phase 3 with stub implementation.
...
3. Validate the initiative ID matches pattern `^[a-zA-Z0-9._-]+$`. If invalid, Print to stdout("Invalid initiative ID. IDs must match pattern: letters, numbers, dots, hyphens, underscores.") and stop.
...
2. If `status` is `completed`, Print to stdout("Initiative '{id}' is already completed. All {N} specs are done.") and stop.
