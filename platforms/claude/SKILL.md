...
3. **Load steering files**: If FILE_EXISTS(`<specsDir>/steering/`) is false, create the directory and foundation templates: Use the Bash tool to run(`mkdir -p <specsDir>/steering`), then for each of product.md, tech.md, structure.md — if FILE_EXISTS(`<specsDir>/steering/<file>`) is false, Use the Write tool to create it with the corresponding foundation template from the Steering Files module. Display a message to the user("Created steering files in `<specsDir>/steering/` — edit them to describe your project. The agent loads these automatically before every spec."). Then load persistent project context from steering files following the Steering Files module. Always-included files are loaded now; fileMatch files are deferred until after affected components and dependencies are identified (step 9).
...
4. **Load memory**: If FILE_EXISTS(`<specsDir>/memory/`) is false, Use the Bash tool to run(`mkdir -p <specsDir>/memory`). Load the local memory layer following the Local Memory Layer module. Decisions, project context, and patterns from prior specs are loaded into the agent's context.
...
1. Use the Bash tool to run(`gh issue create --title "<taskPrefix><TaskTitle>" --body "<TaskDescription>"`)
2. Parse the issue URL/number from stdout
3. Use the Edit tool to modify `tasks.md` — set the task's `**IssueID:**` to the returned issue identifier (e.g., `#42`)

**Jira** (`taskTracking: "jira"`):
1. Use the Bash tool to run(`jira issue create --type=Task --summary="<taskPrefix><TaskTitle>" --description="<TaskDescription>"`)
2. Parse the issue key from stdout (e.g., `PROJ-123`)
3. Use the Edit tool to modify `tasks.md` — set the task's `**IssueID:**` to the returned key

**Linear** (`taskTracking: "linear"`):
1. Use the Bash tool to run(`linear issue create --title "<taskPrefix><TaskTitle>" --description "<TaskDescription>"`)
2. Parse the issue identifier from stdout
3. Use the Edit tool to modify `tasks.md` — set the task's `**IssueID:**` to the returned identifier
...
- **Pending → In Progress**: If IssueID exists and is not `None` or `FAILED`, update the external issue:
...
When `taskTracking` is not `"none"` and the current task has a valid IssueID (not `None` or `FAILED`):
...
2. For each eligible task, check that `**IssueID:**` is populated (not `None` or absent)
...
1. If FILE_EXISTS(`<specsDir>/steering/`) is false:
   ...
   - For each foundation template (product.md, tech.md, structure.md): if FILE_EXISTS(`<specsDir>/steering/<file>`) is false, Use the Write tool to create it with the corresponding foundation template (see Foundation File Templates above)
...
0. If FILE_EXISTS(`<specsDir>/memory/`) is false, Use the Bash tool to run(`mkdir -p <specsDir>/memory`).
...
**Sync failures are non-blocking**: If the Use the Bash tool to run to update the external tracker fails, Display a message to the user with the error and continue. The `tasks.md` state machine is always the source of truth.
