...
3. **Load steering files**: If FILE_EXISTS(`<specsDir>/steering/`), load persistent project context from steering files following the Steering Files module. Always-included files are loaded now; fileMatch files are deferred until after affected components and dependencies are identified (step 9). If `<specsDir>/steering/` does not exist, Display a message to the user: "Tip: Create steering files in `<specsDir>/steering/` (product.md, tech.md, structure.md) to give the agent persistent project context. Run `/specops steering` to set them up, or see the Steering Files module for templates."
...
4. **Load memory**: If FILE_EXISTS(`<specsDir>/memory/`), load the local memory layer following the Local Memory Layer module. Decisions, project context, and patterns from prior specs are loaded into the agent's context. If `<specsDir>/memory/` does not exist, Display a message to the user: "Tip: Run `/specops init` to set up SpecOps with memory, or `/specops memory seed` to populate memory from existing completed specs."
...
5. Set `spec.json` status to `completed`, set `specopsUpdatedWith` to the cached SpecOps version (from the Version Extraction Protocol), update `updated` timestamp (Use the Bash tool to run(`date -u +"%Y-%m-%dT%H:%M:%SZ"`) for the current time), and regenerate `index.json`
...
8. Check if the request is a **map** command (see "Map Subcommand" in the Repo Map module). Patterns: "map", "repo map", "generate map", "refresh map", "show map", "codebase map". These must refer to SpecOps repo map management, NOT a product feature (e.g., "add map component", "map API endpoints", "create sitemap" is NOT map mode). If detected, follow the Map Subcommand workflow instead of the standard phases below.
...
3. If FILE_EXISTS(`.specops.json`), Use the Read tool to read(`.specops.json`) and check for `_installedVersion` and `_installedAt` fields. If present, display:
...
1. For each completed spec in `<specsDir>/` (read from index.json or scan directories):
   - If FILE_EXISTS(`<specsDir>/<spec>/tasks.md`), Use the Read tool to read it.
...
1. If FILE_EXISTS(`.specops.json`), Use the Read tool to read(`.specops.json`) to get `specsDir`; otherwise use default `.specops`.
2. If FILE_EXISTS(`<specsDir>/memory/`) is false: Display a message to the user("No memory found. Memory is created automatically after your first spec completes, or run `/specops memory seed` to populate from existing completed specs.") and stop.
3. If FILE_EXISTS(`<specsDir>/memory/decisions.json`), Use the Read tool to read it and parse.
4. If FILE_EXISTS(`<specsDir>/memory/context.md`), Use the Read tool to read it.
5. If FILE_EXISTS(`<specsDir>/memory/patterns.json`), Use the Read tool to read it and parse.
...
3. Use the Read tool to read(`<specsDir>/index.json`) to get all specs. If index.json does not exist, Use the Glob tool to list(`<specsDir>`), then for each subdirectory check FILE_EXISTS(`<specsDir>/<dir>/spec.json`), and Use the Read tool to read each found `spec.json`.
...
13. Use the Write tool to create(`<specsDir>/memory/context.md`) with the completion summaries built in step 8.
...
1. **Determine specsDir**: If FILE_EXISTS(`.specops.json`), Use the Read tool to read(`.specops.json`) to get `specsDir`; otherwise use default `.specops`.

2. **Discover project files**:
   - If `canAccessGit` is true: Use the Bash tool to run(`git ls-files --cached --others --exclude-standard | head -200`) to get tracked and untracked-but-not-ignored files. This respects `.gitignore` natively.
   - If `canAccessGit` is false: Use the Glob tool to list(`.`) recursively up to depth 3. Then, if FILE_EXISTS(`.gitignore`), Use the Read tool to read(`.gitignore`) and manually exclude matching patterns.
...
3. **Apply scope limits**: Sort files alphabetically by path. If file count exceeds 100, keep the first 100 files and Display a message to the user("Repo map scope limit: showing 100 of {total} files."). Exclude files deeper than 3 directory levels from the project root.
...
7. **Compute source hash**: If `canAccessGit` is true: Use the Bash tool to run(`git ls-files --cached --others --exclude-standard | sort | sha256sum | cut -d' ' -f1`). If `canAccessGit` is false or the command fails, compute a simple hash by concatenating all sorted file paths and noting the count as the hash: `"manual-{file_count}-{first_path_hash}"`.

8. **Get timestamp**: Use the Bash tool to run(`date -u +"%Y-%m-%dT%H:%M:%SZ"`) for the `_generatedAt` field.

9. **Write the repo map**: Use the Write tool to create(`<specsDir>/steering/repo-map.md`) with the frontmatter and body content assembled in the steps above.
...
|------|-----------|-----------|-------------------|-------------------|
| 1 | Python | `*.py` | Use the Bash tool to run(`python3 -c "..."`) using `ast.parse()` | Top-level function signatures (`def name(args)`), class names (`class Name`) |
| 2 | TypeScript/JavaScript | `*.ts`, `*.tsx`, `*.js`, `*.jsx` | Use the Bash tool to run(`grep -n "^export " <path> \| head -10`) | Export declarations (functions, classes, constants, types) |
| 3 | Go, Rust, Java | `*.go`, `*.rs`, `*.java` | Use the Bash tool to run(`grep -n "^func \|^pub fn \|^fn \|^public class \|^public interface " <path> \| head -10`) | Top-level function/method/class declarations |
...
1. If FILE_EXISTS(`<specsDir>/steering/repo-map.md`):
   ...
   - If stale: Display a message to the user("Repo map is stale ({reason}). Refreshing...") and run the Generation algorithm.
   - If fresh: the repo map was already loaded in step 3 as an `inclusion: always` steering file. Continue.

2. If the file does not exist:
   - On interactive platforms (`canAskInteractive = true`): Use the AskUserQuestion tool("No repo map found. Would you like to generate one? This maps your project structure for better spec generation.")
     - If yes: run the Generation algorithm.
     - If no: continue without repo map.
   - On non-interactive platforms (`canAskInteractive = false`): Display a message to the user("Tip: Run `/specops map` to generate a structural repo map for better spec context.") and continue without it.
...
2. If FILE_EXISTS(`<specsDir>/steering/repo-map.md`):
   ...
   - Display current map metadata:
     ...
     ```
   - On interactive platforms: Use the AskUserQuestion tool("Refresh the repo map?")
...
| `canAccessGit: false` | Fall back to Use the Glob tool to list for file discovery. Hash computed from file count + path list. |
...
| `canExecuteCode: true` (all platforms) | Use the Bash tool to run available for `git ls-files`, `grep`, `python3`, `date`, `sha256sum`. |
...
- If FILE_EXISTS returns false AND `canAccessGit` is true: Use the Bash tool to run(`git log --diff-filter=R --summary --oneline -- "<path>"`) to detect renames; Use the Bash tool to run(`git log --diff-filter=D --oneline -- "<path>"`) to detect deletions
...
9. For each repair: Use the Edit tool to modify(`<specsDir>/<name>/tasks.md`) to apply path or status changes.
10. Update `spec.json`: Use the Bash tool to run(`date -u +"%Y-%m-%dT%H:%M:%SZ"`) and Use the Edit tool to modify(`<specsDir>/<name>/spec.json`) to set `updated` to the current timestamp and `specopsUpdatedWith` to the cached SpecOps version (from the Version Extraction Protocol).
11. Regenerate `<specsDir>/index.json` from all `*/spec.json` files.
12. Display a message to the user(`"Reconciliation complete. Applied N fix(es) to <spec-name>."`)
...
1. **Receive plan content**: If plan content was provided inline with the invocation, use it directly. Otherwise, if `canAskInteractive`, Use the AskUserQuestion tool: "Please paste your plan below." If `canAskInteractive` is false and no inline content was provided, Display a message to the user: "From Plan mode requires the plan to be pasted inline. Re-invoke with your plan content included in the request." and stop.
...
7. **Complete**: Proceed to Phase 2 spec review gate (if `config.team.specReview.enabled` or `config.team.reviewRequired`) or Display a message to the user that the spec is ready and they can begin implementation.
...
2. If FILE_EXISTS(`.specops.json`), Use the Read tool to read(`.specops.json`) and check for `_installedVersion` and `_installedAt` fields.
3. Display:
