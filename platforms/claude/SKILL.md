...
8. **Spec dependency gate**: Use the Read tool to read(`<specsDir>/<spec-name>/spec.json`) and check the `specDependencies` array. For each entry with `required: true`, Use the Read tool to read(`<specsDir>/<entry.specId>/spec.json`) and verify `status == "completed"`. If any required dependency is not completed, Display a message to the user("Spec '<spec-name>' has unmet required dependency: '<entry.specId>' (status: <status>). Complete the dependency spec first, or use scope hammering to resolve the blocker.") and STOP. Additionally, run cycle detection (DFS with white/gray/black coloring) across all specs' `specDependencies` — if a cycle is detected, Display a message to the user("Circular dependency detected: {cycle chain}. Resolve before proceeding.") and STOP. If `specDependencies` is absent or empty, this check passes trivially.
