...
3. Read the file at(`<specsDir>/index.json`) to get all specs. If index.json does not exist, List the contents of(`<specsDir>`), then for each subdirectory check FILE_EXISTS(`<specsDir>/<dir>/spec.json`), and Read the file at each found `spec.json`.
...
13. Create the file at(`<specsDir>/memory/context.md`) with the completion summaries built in step 8.
14. Create the file at(`<specsDir>/memory/patterns.json`) with the pattern data built in step 9.
15. Tell the user("Seeded memory from {N} completed specs: {D} decisions, {P} patterns detected.")
...
2. **Discover project files**:
   - If `canAccessGit` is true: Run the terminal command(`git ls-files --cached --others --exclude-standard | head -200`) to get tracked and untracked-but-not-ignored files. This respects `.gitignore` natively.
...
|------|-----------|-----------|-------------------|-------------------|
| 1 | Python | `*.py` | Run the terminal command(`python3 -c "..."`) using `ast.parse()` | Top-level function signatures (`def name(args)`), class names (`class Name`) |
| 2 | TypeScript/JavaScript | `*.ts`, `*.tsx`, `*.js`, `*.jsx` | Run the terminal command(`grep -n "^export " <path> \| head -10`) | Export declarations (functions, classes, constants, types) |
| 3 | Go, Rust, Java | `*.go`, `*.rs`, `*.java` | Run the terminal command(`grep -n "^func \|^pub fn \|^fn \|^public class \|^public interface " <path> \| head -10`) | Top-level function/method/class declarations |
...
1. If FILE_EXISTS(`<specsDir>/steering/repo-map.md`):
   ...
   - If stale: Tell the user("Repo map is stale ({reason}). Refreshing...") and run the Generation algorithm.
   - If fresh: the repo map was already loaded in step 3 as an `inclusion: always` steering file. Continue.

2. If the file does not exist:
   - On interactive platforms (`canAskInteractive = true`): Ask the user("No repo map found. Would you like to generate one? This maps your project structure for better spec generation.")
     - If yes: run the Generation algorithm.
     - If no: continue without repo map.
   - On non-interactive platforms (`canAskInteractive = false`): Tell the user("Tip: Run `/specops map` to generate a structural repo map for better spec context.") and continue without it.
...
2. If FILE_EXISTS(`<specsDir>/steering/repo-map.md`):
   ...
   - Display current map metadata:
     ...
     ```
   - On interactive platforms: Ask the user("Refresh the repo map?")
...
| `canAccessGit: false` | Fall back to List the contents of for file discovery. Hash computed from file count + path list. |
...
| `canExecuteCode: true` (all platforms) | Run the terminal command available for `git ls-files`, `grep`, `python3`, `date`, `sha256sum`. |
