...
2. For each indicator file in the table, Check if the file exists at check
...
7. **Auto-fix** — if `config.dependencySafety.autoFix` is `true`:
   ...
   - Rust: Run the terminal command(`cargo update`)
...
- Run the terminal command(`curl -s --max-time 10 -X POST "https://api.osv.dev/v1/query" -H "Content-Type: application/json" -d '{"package":{"name":"<pkg>","ecosystem":"<ecosystem>"}}'`)
...
**Staleness**: During Phase 1 steering file loading, if `dependencies.md` exists and `_generatedAt` is more than 30 days old, Tell the user("Dependency safety data in `dependencies.md` is over 30 days old. It will be refreshed during the next dependency safety gate run.").
