...
5. Tell the user("Loaded memory: {N} decisions from {M} specs, {P} patterns detected.") — or "No memory files found" if the directory exists but is empty.
...
1. If FILE_EXISTS(`.specops.json`), Read the file at(`.specops.json`) to get `specsDir`; otherwise use default `.specops`.
2. Read the file at(`<specsDir>/index.json`) to get all specs. If index.json does not exist, List the contents of(`<specsDir>`), then for each subdirectory check FILE_EXISTS(`<specsDir>/<dir>/spec.json`), and Read the file at each found `spec.json`.
3. Filter to specs with `status == "completed"`.
4. If no completed specs found: Tell the user("No completed specs found. Complete a spec first, then run seed.") and stop.
...
9. Run the terminal command(`mkdir -p <specsDir>/memory`) if the directory does not exist.
10. Create the file at(`<specsDir>/memory/decisions.json`) with the deduplicated decisions array built in step 6.
11. Create the file at(`<specsDir>/memory/context.md`) with the completion summaries built in step 7.
12. Create the file at(`<specsDir>/memory/patterns.json`) with the pattern data built in step 8.
13. Tell the user("Seeded memory from {N} completed specs: {D} decisions, {P} patterns detected.")
