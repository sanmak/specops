...
3.5. **Check repo map**: After steering files are loaded, check for a repo map following the Repo Map module. **Depth calibration**: If the depth flag is `lightweight`, skip the staleness check and refresh — use the existing repo map if available, or skip if none exists. If the depth flag is `deep`, force a repo map refresh regardless of staleness. For `standard` depth (or when depth is not yet computed), use the default behavior: If Check if the file exists at(`<specsDir>/steering/repo-map.md`), check staleness (time-based and hash-based). If stale, auto-refresh. If the file does not exist, auto-generate it by running the Repo Map Generation algorithm. The repo map is a machine-generated steering file with `inclusion: always` — if it exists and is fresh, it was already loaded in step 3.
...
6. Analyze the user's request to determine type (feature, bugfix, refactor). Also scan for depth override keywords: if the request contains "quick", "lightweight", "simple", "trivial", or "minor", record a depth override of `lightweight`. If the request contains "thorough", "deep", "deep dive", "exhaustive", or "comprehensive", record a depth override of `deep`. If both categories match, the more conservative (deeper) override wins.
...
9.7. **Depth assessment**: Compute the depth flag that calibrates ceremony to complexity. If a depth override was detected in step 6, use the override value. Otherwise, compute from three signals available at this point:
   ...
   - Edit the file at `<specsDir>/<spec-name>/spec.json` to set `"depth": "<flag>"` when the spec is created in Phase 2.
...
5.8. **Dependency introduction gate**: **Depth calibration**: If the depth flag is `lightweight` and no new dependencies are detected in design.md (no install commands or new package references found during scanning), skip this gate — it passes trivially. For `standard` and `deep` depth, execute normally. For `deep` depth, extend the evaluation with additional analysis: check all transitive dependencies of net-new packages for known issues. Execute the Phase 2 Gate Procedure from the Dependency Introduction Gate module (`core/dependency-introduction.md`). Scan design.md for install commands and new package references, compare against the Detected Dependencies in `dependencies.md`, evaluate net-new dependencies using the Build-vs-Install framework and Maintenance Profile Intelligence, surface each to the user via Ask the user, and record all decisions in design.md under `### Dependency Decisions`. Update the Dependency Introduction Policy in `dependencies.md`. If no new dependencies are found, the gate passes trivially. This gate is always active -- there is no config switch to disable it.
...
6.85. **Spec evaluation gate**: **Depth calibration**: If the depth flag is `lightweight`, skip spec evaluation entirely — proceed directly to Phase 3 dispatch. If the depth flag is `standard`, limit spec evaluation to 1 iteration maximum (override `maxIterations` to 1 for this run). If the depth flag is `deep`, run spec evaluation up to the configured `maxIterations`. If `config.implementation.evaluation.enabled` is true (and depth is not `lightweight`), run the Spec Evaluation Protocol from the Adversarial Evaluation module (`core/evaluation.md`). Read the file at the spec's requirements (or `bugfix.md`/`refactor.md`), `design.md`, and `tasks.md`. Score the 4 spec evaluation dimensions (Criteria Testability, Criteria Completeness, Design Coherence, Task Coverage) following the scoring rubric. Create the file at `evaluation.md` with scores and findings. If all dimensions score at or above `config.implementation.evaluation.minScore`, proceed to Phase 3 dispatch. If any dimension fails and iteration count < effective `maxIterations`, revise the failing artifacts using the evaluation feedback and re-evaluate. If iterations exhausted, Tell the user and proceed to Phase 3 with known spec gaps. If evaluation is disabled, skip this step.
...
1. **Implementation gates** — run these checks before any implementation begins:
   ...
   1.5. **Environment pre-flight**: Before updating status, run lightweight environment checks to catch problems early. These checks are unconditional (no config switch to disable):
      - **Test command detection**: Detect the project's test command by checking project config files in priority order:
        1. If Check if the file exists at(`package.json`), Read the file at it. If it contains a `"test"` script under `"scripts"`, use that command.
        2. Else if Check if the file exists at(`pyproject.toml`), Read the file at it. If it contains `[tool.pytest]`, use `pytest`. If it contains `[tool.unittest]`, use `python -m unittest`.
        3. Else if Check if the file exists at(`Makefile`), Read the file at it. If it contains a `test:` target, use `make test`.
        4. Else if Check if the file exists at(`Cargo.toml`), use `cargo test`.
        5. Else if Check if the file exists at(`go.mod`), use `go test ./...`.
        6. Else: Tell the user("No test command detected -- manual test execution required during Phase 3.") and set test_cmd to null.
        Record the detected test command in `implementation.md`.
      - **Dependency installation check**: Based on the project type detected above, check for the expected dependency directory:
        - npm/yarn: Check if the file exists at(`node_modules/`). If missing, Tell the user("Dependencies may not be installed. Run `npm install` (or `yarn install` if Check if the file exists at(`yarn.lock`)) before proceeding.").
        - Python: Check if the file exists at(`.venv/`) or Check if the file exists at(`__pycache__/`). If both missing, Tell the user("Python dependencies may not be installed. Run `pip install -r requirements.txt` or `pip install -e .` before proceeding.").
        - Rust: Check if the file exists at(`target/`). If missing, Tell the user("Rust dependencies may not be built. Run `cargo build` before proceeding.").
        - Go (with vendor): Check if the file exists at(`vendor/`). If missing, Tell the user("Go vendor directory missing. Run `go mod download` before proceeding.").
        This check is non-blocking -- warn and continue.
      - **Git branch state check**: Run the terminal command(`git status --porcelain`). Parse the output:
      ...
      - **Pre-flight summary**: Tell the user with a concise summary: "Environment pre-flight: Test command: [detected or none] | Dependencies: [OK or warning] | Git state: [clean / dirty / BLOCKED: conflicts]".
...
4A.1. **Load evaluation config**: Read the file at `.specops.json` and check `config.implementation.evaluation.enabled`. If evaluation is explicitly disabled (set to false), skip to Phase 4C step 1 (acceptance criteria verification) for backward compatibility. **Depth calibration**: If the depth flag (from `spec.json` or Phase 1 context) is `lightweight`, skip the full adversarial evaluation — proceed directly to Phase 4C step 1 (acceptance criteria verification only). If the depth flag is `deep` and task count > 5 and `config.implementation.evaluation.perTask` is false, consider enabling per-task evaluation for this run to provide granular feedback. If the key is absent or not configured, evaluation is enabled by default. Initialize the evaluation iteration counter to 0.

4A.2. **Run Implementation Evaluation Protocol**: Execute the Implementation Evaluation Protocol from the Adversarial Evaluation module (`core/evaluation.md`). Read the file at all spec artifacts (`requirements.md` or `bugfix.md`/`refactor.md`, `design.md`, `tasks.md`, `implementation.md`) and the files modified during Phase 3. If `canExecuteCode` is true and `config.implementation.evaluation.exerciseTests` is true, Run the terminal command to execute the project's test suite and capture results. Score spec-type-specific dimensions following the evaluation scoring rubric. Edit the file at `<specsDir>/<spec-name>/evaluation.md` to append dimension scores, findings, and remediation instructions for any failing dimensions (preserving prior iteration results). Edit the file at `<specsDir>/<spec-name>/spec.json` to add the `evaluation` object with dimension scores and iteration count.

4A.2.5. **Run Multi-Persona Review Protocol**: Run the Multi-Persona Review Protocol from the Review Agents module (`core/review-agents.md`). Determine active personas (Correctness, Testing, Standards are always active; Security is active only if changed files match security-sensitive patterns). Dispatch each active persona following the platform adaptation rules (sub-agent or inline). Aggregate findings, validate confidence tiers, deduplicate overlapping findings, and determine the review verdict. Append the `## Multi-Persona Review` section to `evaluation.md`. **Depth calibration**: If the depth flag is `lightweight`, skip the multi-persona review.

4A.3. If all dimensions score at or above `config.implementation.evaluation.minScore` AND no P0/P1 HIGH/MODERATE persona findings exist from the multi-persona review: proceed to Phase 4C.
...
4B.4. After remediation completes, increment the evaluation iteration counter and re-enter Phase 4A step 4A.2. **Zero-progress detection**: Read the file at the previous `evaluation.md` scores and compare against the new scores. If no dimension improved by more than 0.5 points compared to the prior iteration, Tell the user("Remediation iteration {N} did not improve evaluation scores. Consider manual intervention.") and proceed to Phase 4C with `evaluation.implementation.passed: false` (preserve `evaluation.spec.passed` from Phase 2) rather than consuming additional iterations.
...
2. **Collect installed packages**: For each detected ecosystem, Read the file at the lock file and extract the list of installed package names:
   - Node.js: Read the file at(`package-lock.json`) or Read the file at(`yarn.lock`) -- extract package names from the `dependencies` or `packages` sections
   - Python: Read the file at(`requirements.txt`) -- extract package names (one per line, strip version specifiers)
   - Rust: Read the file at(`Cargo.lock`) -- extract `name` fields from `[[package]]` sections
   - Ruby: Read the file at(`Gemfile.lock`) -- extract package names from the `GEM > specs` section
   - Go: Read the file at(`go.sum`) -- extract module paths
...
- Bare "explore" with no additional context routes to explore mode, which will Ask the user for the problem statement.
...
1. Read the file at(`.specops.json`) to load config. Apply Safety Rules to all loaded values.
2. If Check if the file exists at(`<specsDir>/steering/`), List the contents of(`<specsDir>/steering/`) and Read the file at each `.md` file with `inclusion: always` in its frontmatter. Store as project context.
3. If Check if the file exists at(`<specsDir>/steering/repo-map.md`), Read the file at(`<specsDir>/steering/repo-map.md`). This is the primary source for grounding approaches in real files.
4. If Check if the file exists at(`<specsDir>/memory/context.md`), Read the file at it for project memory context.
...
1. **Codebase grounding requirement**: Each approach MUST reference at least 2 real files or directories from the repo map. If no repo map is available, Tell the user("No repo map found. Generating approaches based on available project context. Run `/specops map` first for better-grounded suggestions.") and proceed with best-effort generation using any steering files and memory context available.
...
1. If `canAskInteractive` is true:
   Ask the user("Which approach would you like to proceed with? Enter a number (1-N), 'more' for additional approaches, or 'refine' to adjust the problem statement.")
   ...
   - If user says "refine": Ask the user("What would you like to adjust about the problem statement?"). Update the problem statement and return to `generating` phase.
...
2. If `canAskInteractive` is false:
   Tell the user("Explore mode generated N approaches. Review the approaches above and re-invoke with your selected approach number to proceed to spec creation.")
...
2. Tell the user("Proceeding to spec creation with the selected approach as design guidance...")
...
- If the repo map is stale (check `generated` timestamp if available), Tell the user("Repo map may be outdated. Consider running `/specops map` for the most accurate file references.")
...
**Evaluation output format** (presented to user via Ask the user):
...
- Node.js: Run the terminal command(`curl -s --max-time 10 "https://registry.npmjs.org/<package>"`) -- extract `time.modified`, version count, latest version date
- Python: Run the terminal command(`curl -s --max-time 10 "https://pypi.org/pypi/<package>/json"`) -- extract `info.version`, `releases` dates
- Other ecosystems: skip Layer 1, fall through to Layer 2

If the request times out or returns an error, Tell the user("Registry query failed for <package> -- falling through to source repo check.") and proceed to Layer 2.

**Layer 2 -- Source Repository APIs:**

Query the source repository for activity metrics:

- If the package metadata includes a repository URL pointing to GitHub: Run the terminal command(`curl -s --max-time 10 "https://api.github.com/repos/<owner>/<repo>"`) -- extract `stargazers_count`, `pushed_at`, `open_issues_count`
- If no repository URL or not on GitHub: skip to Layer 3

If the request times out or returns an error, Tell the user("Source repo query failed for <package> -- falling through to LLM assessment.") and proceed to Layer 3.
...
1. Read the file at(`<specsDir>/<spec-name>/design.md`) and scan for:
...
2. Read the file at(`<specsDir>/steering/dependencies.md`) to get the Detected Dependencies list (auto-populated by the dependency safety gate).
...
4. For each net-new dependency (maximum 10):
   ...
   c. Ask the user with the evaluation output, asking for approval or rejection:
...
6. Edit the file at(`<specsDir>/<spec-name>/design.md`) to write the Dependency Decisions table.
...
1. Read the file at(`<specsDir>/<spec-name>/design.md`) and locate the `### Dependency Decisions` section
...
4. If NOT approved (missing from table, or Decision is Rejected): this is a **protocol breach**. Tell the user("Protocol breach: attempting to install unapproved dependency '<package>'. This package is not listed in design.md ### Dependency Decisions. Options: (1) Add it to the spec by re-running the dependency introduction gate, (2) Remove the install and build the functionality in-house.") and HALT until the user resolves the situation.

**No Dependency Decisions section:** If design.md has no `### Dependency Decisions` section, any install command is a protocol breach -- the dependency introduction gate was skipped or the spec predates this feature. For backward compatibility with pre-existing specs (specs with `specopsCreatedWith` earlier than the version that introduced this gate): Tell the user with a warning but allow the install to proceed. For specs created with the current version or later: enforce as a protocol breach.

**Post-Phase-3 verification:**

After all tasks are completed but before Phase 3 exit:

1. Read the file at(`<specsDir>/<spec-name>/design.md`) and extract all packages with `Decision: Approved` from the Dependency Decisions table
2. For each approved package, verify it was actually installed by checking the project's manifest or lock files
3. If an approved package was NOT installed: Tell the user("Warning: dependency '<package>' was approved in design.md but was not installed during implementation. This may indicate a phantom approval or a change in approach. Please confirm this is intentional.") -- this is a warning, not a blocking error
...
3. Edit the file at(`<specsDir>/steering/dependencies.md`) to add:
...
- For approved dependencies: Edit the file at to add an entry under `### Approved Patterns` with the package category and rationale (e.g., "HTTP server frameworks: approved when scope requires request handling")
- For rejected dependencies: Edit the file at to add an entry under `### Rejected Patterns` with the rejection reason (e.g., "Utility libraries for single functions: prefer native/built-in alternatives")
...
1. Read the file at the requirements file (requirements.md, bugfix.md, or refactor.md), design.md, and tasks.md.
...
1. Read the file at the requirements file, design.md, tasks.md, and implementation.md.
2. Read the file at each file listed in the implementation.md Session Log "Files to Modify" entries to inspect the actual implementation.
3. If `canExecuteCode` is true AND `config.implementation.evaluation.exerciseTests` is true: Run the terminal command to execute the project's test suite. Record test output (pass count, fail count, specific failures).
...
11. If ANY dimension scores below `minScore` AND current iteration >= `maxIterations`: Tell the user("Implementation evaluation did not pass after {iterations} iterations. Proceeding to completion with known quality gaps: {list of failing dimensions and scores}.") and proceed to Phase 4C.
...
2. Assess Fix Determinism: Read the file at the finding's evidence and remediation instruction. If the remediation specifies a single, unambiguous change (add line X, rename Y to Z, delete line N), classify as deterministic. If the remediation describes a strategy or lists alternatives, classify as non-deterministic.
...
2. For each auto_fix finding, in order:
   a. Edit the file at to apply the fix as specified in the finding's remediation instruction.
   b. If `canExecuteCode` is true, Run the terminal command the project's test suite. If any test that was previously passing now fails, the auto-fix introduced a regression.
...
2. If `canAskInteractive` is true: present all gated fixes as a single batch to the developer via Ask the user. For each finding, show: finding ID, description, proposed change (file path + change summary), and risk assessment. The developer can approve all, reject all, or selectively approve individual fixes.
...
1. `manual` findings: Tell the user with each manual finding's ID, severity, confidence, evidence, and description. Note that no agent fix is attempted.
...
1. Read the file at `<specsDir>/<spec-name>/evaluation.md` to get the failing dimensions and remediation instructions.
...
1. Read the file at `<specsDir>/<spec-name>/evaluation.md` to get the failing dimensions, remediation instructions, and (if present) persona findings with finding IDs, evidence, and remediation instructions.
...
- Tell the user("Evaluation feedback loop made no progress — scores unchanged after iteration {N}. Stopping to avoid infinite loop.")
...
1. **Steering files**: If Check if the file exists at(`<specsDir>/steering/`), scan steering files for a "Security-Sensitive Files" heading and extract the file list.
2. **Project documentation**: If Check if the file exists at(`CLAUDE.md`), Read the file at it and extract paths listed under a "Security-Sensitive Files" heading.
...
1. **Determine active personas**: For each persona in the registry, evaluate its triggering conditions to determine activation. Record an activation reason for every persona.
   ...
   c. For each persona:
      ...
      - Else if `activationMode` is `"conditional"`:
        ...
        iii. If no filePattern matched and the persona has `contentPatterns`: Read the file at each changed file and match content against the `contentPatterns` regex patterns.
...
**Pre-cycle spec evaluation (one-time):** Before entering the cycle loop, if Read the file at(`.specops.json`) shows `config.implementation.evaluation.enabled` is `true` (default: true), run spec evaluation once since the spec already exists and does not change during pipeline execution:

1. Read the file at(`<specsDir>/<spec-name>/requirements.md`) (or `bugfix.md`/`refactor.md`), Read the file at(`<specsDir>/<spec-name>/design.md`), and Read the file at(`<specsDir>/<spec-name>/tasks.md`).
2. Apply the adversarial spec evaluator against the collected artifacts and Create the file at the results to `<specsDir>/<spec-name>/evaluation.md`.
3. Read the file at(`<specsDir>/<spec-name>/evaluation.md`) and check the overall verdict. If the verdict is `fail`, Tell the user("Spec evaluation failed before pipeline start. Review evaluation.md for findings.") and STOP — do not enter the cycle loop. If the verdict is `pass`, proceed to the cycle loop.
...
while cycle < maxCycles:
    ...
    if evaluationEnabled:
        ...
        if evaluationResponse is valid JSON with status, scores, verdict, findings:
            ...
            // Create the file at <specsDir>/<spec-name>/evaluation.md with formatted results
        else:
            // Fallback: evaluation did not return valid headless JSON
            // Read evaluation.md and parse markdown (backward compatibility)
            // Read the file at <specsDir>/<spec-name>/evaluation.md
        ...
        if overallVerdict == "pass":
            ...
            Tell the user("Pipeline completed in {cycle} cycle(s). All evaluation criteria passed.")
            break

        // Zero-progress detection (evaluation-based)
        if evaluationScores == previousEvaluationScores:
            Tell the user("No progress in cycle {cycle} — evaluation scores unchanged from previous cycle. Stopping to avoid infinite loop.")
    ...
    if cycle == maxCycles:
        if evaluationEnabled:
            Tell the user("Pipeline reached max cycles ({maxCycles}). Evaluation still failing on: {failingCategories}. Manual intervention required.")
        else:
    ...
    if evaluationEnabled:
        ...
        Tell the user("Cycle {cycle}/{maxCycles}: evaluation failing on {failingCategories}. Starting next cycle...")
    else:
