...
1. Run the terminal command(`gh issue create --title "<taskPrefix><TaskTitle>" --body "<TaskDescription>"`)
2. Parse the issue URL/number from stdout
3. Edit the file at `tasks.md` — set the task's `**IssueID:**` to the returned issue identifier (e.g., `#42`)

**Jira** (`taskTracking: "jira"`):
1. Run the terminal command(`jira issue create --type=Task --summary="<taskPrefix><TaskTitle>" --description="<TaskDescription>"`)
2. Parse the issue key from stdout (e.g., `PROJ-123`)
3. Edit the file at `tasks.md` — set the task's `**IssueID:**` to the returned key

**Linear** (`taskTracking: "linear"`):
1. Run the terminal command(`linear issue create --title "<taskPrefix><TaskTitle>" --description "<TaskDescription>"`)
2. Parse the issue identifier from stdout
3. Edit the file at `tasks.md` — set the task's `**IssueID:**` to the returned identifier
...
**Sync failures are non-blocking**: If the Run the terminal command to update the external tracker fails, Tell the user with the error and continue. The `tasks.md` state machine is always the source of truth.
