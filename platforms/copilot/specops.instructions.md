...
3. **Load steering files**: If FILE_EXISTS(`<specsDir>/steering/`), load persistent project context from steering files following the Steering Files module. Always-included files are loaded now; fileMatch files are deferred until after affected components and dependencies are identified (step 8). If `<specsDir>/steering/` does not exist, Tell the user: "Tip: Create steering files in `<specsDir>/steering/` (product.md, tech.md, structure.md) to give the agent persistent project context. Run `/specops steering` to set them up, or see the Steering Files module for templates."
...
3. **Documentation check**: Identify project documentation that may need updating based on files modified during implementation:
   ...
   - **New subcommand check**: If this spec shipped a new `/specops` subcommand (a new command branch in Getting Started or a new module routed from there):
     - [ ] `canAskInteractive = false` fallback written for every `Ask the user` branch in the new subcommand
...
3. For each target spec:
   a. If FILE_EXISTS(`<specsDir>/<name>/spec.json`), Read the file at(`<specsDir>/<name>/spec.json`) to load metadata. If not found, Tell the user(`"Spec '<name>' not found in <specsDir>. Run List the contents of(<specsDir>) to see available specs."`) and stop.
...
7. Ask the user(`"Which findings to fix? Enter 'all', comma-separated numbers (e.g. '1,3'), or 'skip' to exit."`)
...
10. Update `spec.json.updated` timestamp: Run the terminal command(`date -u +"%Y-%m-%dT%H:%M:%SZ"`) and Edit the file at(`<specsDir>/<name>/spec.json`) with the new timestamp.
