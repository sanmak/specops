...
**Pre-cycle spec evaluation (one-time):** Before entering the cycle loop, if Use the Read tool to read(`.specops.json`) shows `config.evaluation.enabled` is `true`, run spec evaluation once since the spec already exists and does not change during pipeline execution:
