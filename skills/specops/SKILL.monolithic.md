...
9.7. **Depth assessment**: Compute the depth flag that calibrates ceremony to complexity. If a depth override was detected in step 6, use the override value. Otherwise, compute from three signals available at this point:
   ...
   - Use the Edit tool to modify `<specsDir>/<spec-name>/spec.json` to set `"depth": "<flag>"` when the spec is created in Phase 2.
...
1. **Implementation gates** — run these checks before any implementation begins:
   ...
   1.5. **Environment pre-flight**: Before updating status, run lightweight environment checks to catch problems early. These checks are unconditional (no config switch to disable):
      - **Test command detection**: Detect the project's test command by checking project config files in priority order:
        1. If Use the Bash tool to check if the file exists at(`package.json`), Use the Read tool to read it. If it contains a `"test"` script under `"scripts"`, use that command.
        2. Else if Use the Bash tool to check if the file exists at(`pyproject.toml`), Use the Read tool to read it. If it contains `[tool.pytest]`, use `pytest`. If it contains `[tool.unittest]`, use `python -m unittest`.
        3. Else if Use the Bash tool to check if the file exists at(`Makefile`), Use the Read tool to read it. If it contains a `test:` target, use `make test`.
        4. Else if Use the Bash tool to check if the file exists at(`Cargo.toml`), use `cargo test`.
        5. Else if Use the Bash tool to check if the file exists at(`go.mod`), use `go test ./...`.
        6. Else: Display a message to the user("No test command detected -- manual test execution required during Phase 3.") and set test_cmd to null.
      ...
      - **Dependency installation check**: Based on the project type detected above, check for the expected dependency directory:
        - npm/yarn: Use the Bash tool to check if the file exists at(`node_modules/`). If missing, Display a message to the user("Dependencies may not be installed. Run `npm install` (or `yarn install` if Use the Bash tool to check if the file exists at(`yarn.lock`)) before proceeding.").
        - Python: Use the Bash tool to check if the file exists at(`.venv/`) or Use the Bash tool to check if the file exists at(`__pycache__/`). If both missing, Display a message to the user("Python dependencies may not be installed. Run `pip install -r requirements.txt` or `pip install -e .` before proceeding.").
        - Rust: Use the Bash tool to check if the file exists at(`target/`). If missing, Display a message to the user("Rust dependencies may not be built. Run `cargo build` before proceeding.").
        - Go (with vendor): Use the Bash tool to check if the file exists at(`vendor/`). If missing, Display a message to the user("Go vendor directory missing. Run `go mod download` before proceeding.").
        This check is non-blocking -- warn and continue.
      - **Git branch state check**: Use the Bash tool to run(`git status --porcelain`). Parse the output:
        - If the command fails (not a git repo), skip this check silently.
        - If any output lines start with `UU`, `AA`, `DD`, `AU`, or `UA`: merge conflicts detected. Display a message to the user("Merge conflicts detected in: [file list]. Resolve conflicts before implementation.") and STOP -- do not proceed to implementation.
        - If output is non-empty but no conflict markers: Display a message to the user("Working tree has uncommitted changes.") -- non-blocking warning, continue.
        - If output is empty: clean state.
      - **Pre-flight summary**: Display a message to the user with a concise summary: "Environment pre-flight: Test command: [detected or none] | Dependencies: [OK or warning] | Git state: [clean / dirty / BLOCKED: conflicts]".
...
3. Use the Bash tool to run(`export SPECOPS_TITLE="[{category}] {sanitized_title}" && gh issue create --repo sanmak/specops --title "$SPECOPS_TITLE" --label "{label}" --body-file "{tmpfile}"`)
...
2. **User-level install** (Claude only): Check Use the Bash tool to check if the file exists at for `~/.claude/skills/specops/SKILL.md`. If present, the installation method is **Claude user-level install**. Note: `~` resolves to the user's home directory; if the platform cannot resolve this path, skip this check and fall through.
3. **Project-level install**: Check Use the Bash tool to check if the file exists at for platform-specific paths in the current project:
...
4. **Local clone**: Check Use the Bash tool to check if the file exists at for `generator/generate.py` in the current directory. If present, the user is running from a cloned SpecOps repository.
...
1. Display a message to the user that the update is complete.
...
2. For each auto_fix finding, in order:
   a. Use the Edit tool to modify to apply the fix as specified in the finding's remediation instruction.
   b. If `canExecuteCode` is true, Use the Bash tool to run the project's test suite. If any test that was previously passing now fails, the auto-fix introduced a regression.
...
1. `manual` findings: Display a message to the user with each manual finding's ID, severity, confidence, evidence, and description. Note that no agent fix is attempted.
...
1. Use the Read tool to read `<specsDir>/<spec-name>/evaluation.md` to get the failing dimensions, remediation instructions, and (if present) persona findings with finding IDs, evidence, and remediation instructions.
...
1. **Steering files**: If Use the Bash tool to check if the file exists at(`<specsDir>/steering/`), scan steering files for a "Security-Sensitive Files" heading and extract the file list.
2. **Project documentation**: If Use the Bash tool to check if the file exists at(`CLAUDE.md`), Use the Read tool to read it and extract paths listed under a "Security-Sensitive Files" heading.
...
1. **Determine active personas**: For each persona in the registry, evaluate its triggering conditions to determine activation. Record an activation reason for every persona.
   ...
   c. For each persona:
      ...
      - Else if `activationMode` is `"conditional"`:
        ...
        iii. If no filePattern matched and the persona has `contentPatterns`: Use the Read tool to read each changed file and match content against the `contentPatterns` regex patterns.
...
while cycle < maxCycles:
    ...
    if evaluationEnabled:
        ...
        if evaluationResponse is valid JSON with status, scores, verdict, findings:
            ...
            // Use the Write tool to create <specsDir>/<spec-name>/evaluation.md with formatted results
        else:
            // Fallback: evaluation did not return valid headless JSON
            // Read evaluation.md and parse markdown (backward compatibility)
            // Use the Read tool to read <specsDir>/<spec-name>/evaluation.md
        ...
        // For each auto_fix finding: Use the Edit tool to modify to apply fix, verify no test regression
        // If auto_fix fails: revert, reclassify as gated_fix

        // Handle gated_fix items based on platform capability
        if canAskInteractive:
            // Batch all gated_fix items and Use the AskUserQuestion tool for approval
            // Apply approved fixes, reclassify rejected as manual
        else:
            // Non-interactive: treat gated_fix as auto_fix (apply without asking)

        // Report manual findings to developer via Display a message to the user (no fix attempted)
...
- **Never fabricate timestamps.** All ISO 8601 timestamps in `spec.json` must come from the system clock via Use the Bash tool to run(`date -u +"%Y-%m-%dT%H:%M:%SZ"`). Invariant: `updated` >= `created`.
...
User: "/specops interview I want to build something for restaurants"
