...
0. **Phase 2 entry gate**: Use the Read tool to read `implementation.md` and verify it contains `## Phase 1 Context Summary`. If missing, STOP — return to Phase 1 step 5.5 and write it. Proceeding without the Context Summary is a protocol breach.
...
1. **Receive plan content**: Resolve plan content using the first matching branch:
   ...
   **Branch B — File path**: If a file path was provided with the invocation (e.g., `from-plan <path>`), validate the path before reading:
   ...
   - Check FILE_EXISTS(`<path>`). If the file does not exist, Display a message to the user: "Plan file not found: `<path>`" and stop.
   ...
   **Branch C — Platform auto-discovery**: If no content and no path were provided, and the platform configuration includes a `planFileDirectory` field:
   ...
   - Once the user selects a file, validate the path (no `../`, must be `.md`, FILE_EXISTS check) and Use the Read tool to read it.
...
4. If `"auto"`: Use the Read tool to read `tasks.md` and compute a complexity score for pending tasks:
...
1. **Select next task (dependency-aware)**: Use the Read tool to read `tasks.md` — parse all tasks with their statuses and `**Dependencies:**` fields. Build a ready set: tasks with `**Status:** Pending` whose dependencies are all `Completed` or `None`. From the ready set, select by priority (`High` > `Medium` > `Low`), then by document order (lower task number first). If the ready set is empty but Pending tasks remain, Display a message to the user with a dependency deadlock warning and pause for manual intervention.
...
5. When the agent returns:
   ...
   a.5. **Quality gate** (if status is `Completed`): Check for degradation signals before accepting the result:
      - **File existence**: For each path in the task's "Files to Modify", FILE_EXISTS the path. If any file was supposed to be created but does not exist, Display a message to the user with warning and set the task back to `In Progress` for re-evaluation.
