...
7. **Implementation evaluation-to-remediation dispatch**: After Phase 4A writes evaluation results to `<specsDir>/<spec-name>/evaluation.md`, the dispatcher checks for implementation failures:
   ...
   - If failures are detected:
     ...
     - `canDelegateTask: false` and `canAskInteractive: true`: Use the Write tool to create the remediation context to `<specsDir>/<spec-name>/implementation.md` (appended as `## Remediation Context`) and Display a message to the user("Implementation evaluation found failures. Remediation context written to implementation.md. Start a fresh session to address the failing criteria.").
     - `canDelegateTask: false` and `canAskInteractive: false`: Use the Write tool to create the remediation context to implementation.md (appended as `## Remediation Context`) and continue execution sequentially with the constrained task scope.
