...
6. **Spec evaluation dispatch at Phase 2 exit**: When the dispatcher detects `## Phase 2 Completion Summary` in `implementation.md` AND Use the Read tool to read(`.specops.json`) shows `config.evaluation.enabled` is `true`, dispatch a spec evaluation step before Phase 3 begins:
